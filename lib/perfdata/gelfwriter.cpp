...
void GelfWriter::ReconnectInternal()
{
	...
	if (ssl) {
		...
		m_Stream.first = Shared<AsioTlsStream>::Make(IoEngine::Get().GetIoService(), *sslContext, GetHost());
	} else {
		m_Stream.second = Shared<AsioTcpStream>::Make(IoEngine::Get().GetIoService());
	}
